      ...
      <div className="rounded-lg border border-border bg-card p-6 shadow-sm">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <p className="text-sm font-mono text-muted">{complaint._id}</p>
          </div>
        ...
        {complaint.imageUrl && complaint.imageUrl.length > 0 && (
          <img
            src={complaint.imageUrl[0] || complaint.imageUrl || "/placeholder.svg"}
            alt={complaint.title}
            className="mb-4 h-64 w-full rounded-md border border-border object-cover"
          />
        )}
        ...
        <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
          <Info icon={<MapPin size={16} />} label="Address" value={complaint.location?.address || "N/A"} />
          <Info icon={<Calendar size={16} />} label="Created" value={new Date(complaint.createdAt).toLocaleDateString()} />
          <Info icon={<Building size={16} />} label="Department (AI Selected)" value={complaint?.department?.name || "Not Allocated"} />
          <Info
            icon={<User size={16} />}
            label="Assigned Official"
            value={complaint?.assignedOfficial?.name || "Not assigned"}
          />
        </div>

        {/* Resolution section */}
        {(complaint.resolutionNote || (complaint.resolutionImages && complaint.resolutionImages.length > 0)) && (
